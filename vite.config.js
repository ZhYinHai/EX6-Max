...
function wordpressTemplatePreview() {
  return {
    ...
    transformIndexHtml(html) {
      const readPartial = (partialPath) => fs.readFileSync(partialPath, 'utf8').replace(/^<\?php[\s\S]*?\?>\s*/, '');
      ...
      const template = fs
        ...
        .replace(
          "<?php require PHANTEKS_EX6_PATH . 'templates/sections/telemetry-simulator.php'; ?>",
          telemetryTemplate,
        )
        .replace(/^<\?php[\s\S]*?\?>\s*/, '');
